...
export default function ARScene({ videoUrl, targetUrl }: ARSceneProps) {
  ...
  // Detect iOS for autoplay restriction
  useEffect(() => {
  ...
  useEffect(() => {
    const initAR = async () => {
      try {
        ...
        setHasError("Falha ao inicializar a câmera. Tente recarregar a página.");
        setIsLoading(false);
      }
    };
    ...
    return (
      <div className="fixed inset-0 bg-guestalt-black flex items-center justify-center p-4">
        <div className="text-center">
          ...
          <p className="text-white text-lg mb-4">{hasError}</p>
          <button
    ...
    <div className="ar-container">
      ...
      <div
        dangerouslySetInnerHTML={{
          __html: `
            <a-scene
              mindar-image="imageSrcSrc: ${targetUrl}; autoStart: true; uiLoading: no; uiError: no; uiScanning: yes;"
              color-space="sRGB"
              embedded
